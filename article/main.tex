...
Given an integer $k \ge 3$ and a real $c$, we define the interger linear program $P_{c,k}$ as follows:
\[
\begin{array}{rrcll}
	(P_{k,c}) &\max& 1&&\\
	...
	\forall i \ge 1, su_i = 1\Rightarrow &d_i&=&d_{i-1} - 2x_i&\\
	\forall i \ge 1, su_i = 1\Rightarrow &a_{i, j}&\ge&a_{i-1, j+1}& j = 1,\ldots,k-1\\
	\forall i \ge 1, su_i = 1\Rightarrow &a_{i, -2}&\ge&x_i&\\
...
\begin{enumerate}
	...
	We use
	$a_{i,-j}$ as a shorthand notation for $a_{i,k+1-j}$.
...
\end{enumerate}
The constains of the type ``$su_i = b$ implies $C$'' can be encoded using the so-called ``Big M method''\footnote{See \url{https://en.wikipedia.org/wiki/Big\_M\_method}.}. 
The constaint $d_k < d_0$ can be approximately encoded as $d_k \le d_0 - \delta$ for some small $\delta > 0$.
